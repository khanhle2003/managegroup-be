...
public interface thongkedvmRepo extends JpaRepository<DataofCusEntity, Integer> {
    ...
    @Query(value = "SELECT * FROM data2 WHERE invitation_unit IN :invitationUnits AND start_date LIKE %:year%", 
           nativeQuery = true)
    ...
    @Query(value = "SELECT COUNT(*) FROM data2 WHERE invitation_unit IN :invitationUnits AND start_date LIKE %:year%", 
           nativeQuery = true)
