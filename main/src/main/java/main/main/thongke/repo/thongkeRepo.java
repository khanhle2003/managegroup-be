...
public interface thongkeRepo extends JpaRepository<DataofCusEntity, Integer> {
    ...
    @Query(value = "SELECT * FROM data2 WHERE country IN :countries AND start_date LIKE %:year%", 
           nativeQuery = true)
    ...
    @Query(value = "SELECT COUNT(*) FROM data2 WHERE country IN :countries AND start_date LIKE %:year%", 
           nativeQuery = true)
